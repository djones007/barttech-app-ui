...
3b. **Exclude the mount from the app's `tsconfig.json` `exclude` array too** (`"src/app-ui"`, alongside the `"src/web-core"` that is already there). **This is not the same step as 3 and was missed estate-wide** — checked 2026-08-20, all 11 consumers excluded `src/web-core` and *none* excluded `src/app-ui`.

   The consequence is not hypothetical. `contrast.test.ts` imports `node:test`, and a consumer's `include` of `**/*.ts` pulls it into that app's type-check: confirmed with `tsc --listFiles`, which listed `contrast.test.ts` in the root file set of an app that has nothing to do with it. It passed only because a Next app happens to already ship `@types/node`. Drop that dependency, or add a test file here needing anything a consumer lacks, and eleven unrelated builds break on a file none of them own — **the exact failure described in "Why this is not in the shared framework-free core module" above, which had already happened once to the other shared submodule.**

   **Excluding does NOT stop real errors being caught, which is why this is safe.** `exclude` only removes files from the *root* set; anything the app imports is still loaded and type-checked as a dependency. Measured before/after on a real consumer: `contrast.test.ts` and `contrast.ts` dropped out (nothing imports them), while `LeftNav.tsx`, `DataTable.tsx`, `BulkActions.tsx` and `DateRangePicker.tsx` all remained. A genuine type error at the consumer boundary still fails the build.
