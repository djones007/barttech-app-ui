...
//
// Promoted out of an earlier shared template on 2026-07-31. Several consumer
// repos carried byte-identical copies — promoted before they diverged rather
// than after, which is the only cheap moment to do it.
//
...
//
// 2026-08-01: that change left a hole. When `getRowId` was omitted the default
// was `String(row.id ?? "")`, so a row set with no `id` gave EVERY row the key
// `""` — one tick selected the page, the bar read "1 selected", and the action
// ran on all of it. Now `createRowIdFallback()` keys those rows by object
// identity (unique, sort-safe, clears on refetch) and warns in dev. Still pass
// `getRowId` when the rows have a real id.
// ---------------------------------------------------------------------------
...
export type DataTableProps<T extends Record<string, unknown>> = {
   ...
   *
   * Pass it whenever the rows have no `id` field. Without it, selection falls
   * back to the row object's identity: unique and safe to sort/filter/page
   * against, but it does NOT survive the data being refetched — the selection
   * clears rather than carrying over. Dev builds log once when that happens.
   * Index-keyed selection is never used; it is a correctness bug, not a
   * shortcut.
   */
   ...
   *
   * `onDelete` confirms before it runs (changed 2026-08-01 — it used to pass
   * `confirm: false` on the assumption that every caller ran its own dialog,
   * which nothing in the estate actually did). If your handler already
   * confirms, drop this prop and declare the action yourself with
   * `actions={[{ key: "delete", danger: true, confirm: false, … }]}` — the
   * generic array is the full-control path and this is the shorthand.
   */
  onEdit?: (rows: T[]) => void;
...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const allActions: BulkAction<T>[] = useMemo(() => {
    ...
    // No `confirm: false` here any more. It was set on the belief that legacy
    // onDelete callers ran their own dialog; checked across every known
    // consumer on 2026-08-01 and nothing used this prop that way (real
    // deletes go through `actions` with explicit `confirm`), so the only
    // thing it bought was an unconfirmed bulk delete waiting for the next
    // consumer. `danger` now supplies the default confirmation; opt out via
    // `actions`.
    if (onDelete) legacy.push({ key: "delete", label: "Delete", icon: IconTrash, danger: true, row: true, run: onDelete });
  ...
  // One fallback per table instance, created lazily: the generated keys live in
  // its closure, so rebuilding it every render would hand the same row a new
  // key each time and nothing would ever stay selected.
  const [fallbackRowId] = useState(() => createRowIdFallback<T>("DataTable"));
  const rowId = useMemo(() => getRowId ?? fallbackRowId, [getRowId, fallbackRowId]);
